...
我不能简单地说"鼠标箭头引入了一个新的输入焦点因此鼠标有罪"，毕竟现在部分 CLI 应用的复杂度也有了充分提升。我自己每天就在用两层 tumx，里面还有 weechat 和 Vim，都是多 tab/window 界面。加上无法避免的多应用协作，输入焦点切换总归要提到日程上来。加之软件界面对 context 的利用还太少，日渐增加的功能统一塞在屏幕边缘的工具栏/菜单/面板里，鼠标箭头只能疲于奔命（好像成语用得不是很恰当）。
